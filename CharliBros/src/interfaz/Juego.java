...
public class Juego extends javax.swing.JFrame {
    ...
    private void jFlappyKeyReleased(java.awt.event.KeyEvent evt) {
        if (empezar) {
            if (evt.getExtendedKeyCode() == 32) {
                this.mvnt_Charli.setVelocidad(aumentarVelocidad());
                this.mvnt_Charli.setSaltar(true);
                jCharli.requestFocus(true);
                Sonido.saltar();
            }
            validarChoqueTubos();
        }
    ...
    public void Empezar() {
        mvnt_tubos = new MovimientoTubos();
        mvnt_Charli = new MovimientoCharli(this);
        mvnt_tubos.start();
        mvnt_Charli.start();
        empezar = true;
        jCharli.requestFocus();
        this.setTitle("CHARLI BROS - " + nombre);
    }

    public void validarChoqueTubos() {
        Point loclz_Charli = jCharli.getLocation();
        ...
        if (loclz_Charli.x > (loclz_Tubo1.x - 32) && loclz_Charli.x < ((loclz_Tubo1.x - 32) + 82) && loclz_Charli.y < (loclz_Tubo1.y + 235)) {
            Sonido.choque();
            this.mvnt_tubos.stop();
            this.mvnt_Charli.setSaltar(false);
            empezar = false;
            Sonido.caida();
        } else if (loclz_Charli.x > (loclz_Tubo2.x - 32) && loclz_Charli.x < ((loclz_Tubo2.x - 32) + 82) && loclz_Charli.y < (loclz_Tubo2.y + 235)) {
            Sonido.choque();
            this.mvnt_tubos.stop();
            this.mvnt_Charli.setSaltar(false);
            empezar = false;
            Sonido.caida();
        } else if (loclz_Charli.x > (loclz_Tubo3.x - 32) && loclz_Charli.x < ((loclz_Tubo3.x - 32) + 82) && loclz_Charli.y > (loclz_Tubo3.y - 22)) {
            Sonido.choque();
            this.mvnt_tubos.stop();
            this.mvnt_Charli.setSaltar(false);
            empezar = false;
            Sonido.caida();
        } else if (loclz_Charli.x > (loclz_Tubo4.x - 32) && loclz_Charli.x < ((loclz_Tubo4.x - 32) + 82) && loclz_Charli.y > (loclz_Tubo4.y - 22)) {
            Sonido.choque();
            this.mvnt_tubos.stop();
            this.mvnt_Charli.setSaltar(false);
            empezar = false;
        ...
        if (y == 448) {
            ...
            try {
                Thread hilo = new Thread() {
                    ...
                    public void run() {
                        mvnt_tubos.stop();
                        MovimientoCharli.detenerHilo = true;
                        stop();
                    }
                };
                hilo.start();
    ...
    public void EmpezarNuevo() {
        jCharli.setLocation(posicionCharli);
        jPuntaje.setText("0");
